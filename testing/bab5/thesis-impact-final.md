...
| Judul Multivariate Tests berpasangan | Multivariate Tests — Hotelling T² Berpasangan | Multivariate Tests (baris efek kemudian menjadi "Hotelling T² (Paired)", lihat §8) |
...
## 8. Penyeragaman teks ke bahasa Inggris (2026-09-26)

Teks tampilan modul GLM Multivariate yang masih berbahasa Indonesia diseragamkan ke bahasa Inggris, seperti modul Statify lain. Modul Repeated Measures sudah seluruhnya berbahasa Inggris. Perhitungan, payload, respons WASM, urutan tabel, dan kapan pesan muncul tidak berubah. Gambar di `gambar/` dan `sleeping-dog/` sudah dibuat ulang dari build `JyX0CCEXNcyYSCxSLCIPP`. Yang perlu disesuaikan di naskah:
