...
describe("formatOrdinalResult", () => {
  ...
  it("should format all required tables correctly when present", () => {
    ...
    const modelFitting = formatted.sections.find(
      (s) => s.id === "ordinal_model_fitting_information",
    );
    ...
    const gof = formatted.sections.find(
      (s) => s.id === "ordinal_goodness_of_fit",
    );
    ...
    const pseudo = formatted.sections.find(
      (s) => s.id === "ordinal_pseudo_r_square",
    );
    ...
    const params = formatted.sections.find(
      (s) => s.id === "ordinal_parameter_estimates",
    );
    ...
    const parallel = formatted.sections.find(
      (s) => s.id === "ordinal_test_of_parallel_lines",
    );
    ...
    const iterHistory = formatted.sections.find(
      (s) => s.id === "ordinal_iteration_history",
    );
  ...
  it("should render saved variables and parameter estimates when both are requested", () => {
    ...
    const saved = formatted.sections.find(
      (s) => s.id === "ordinal_saved_variables",
    );
    const params = formatted.sections.find(
      (s) => s.id === "ordinal_parameter_estimates",
    );
  ...
  it("should render only saved variables when parameter estimates are not requested", () => {
    const formatted = formatOrdinalResult({
      ...
      savedVariableOptions: { predictedResponseCategory: true },
      savedVariables: {
        columns: [{ name: "PRE_1", label: "Predicted Response Category" }],
      },
      parameterEstimates: [
        { group: "Threshold", variable: "[y = 1]", estimate: -1.2 },
      ],
    ...
    expect(
      formatted.sections.find((s) => s.id === "ordinal_saved_variables"),
    ).toBeDefined();
    expect(
      formatted.sections.find((s) => s.id === "ordinal_parameter_estimates"),
    ).toBeUndefined();
  });
  ...
  it("should add selected link function notes to core ordinal tables", () => {
    ...
    expect(
      formatted.sections.find(
        (s) => s.id === "ordinal_model_fitting_information",
      )?.note,
    ).toBe("Link function: Probit.");
    expect(
      formatted.sections.find((s) => s.id === "ordinal_goodness_of_fit")?.note,
    ).toBe("Link function: Probit.");
    expect(
      formatted.sections.find((s) => s.id === "ordinal_pseudo_r_square")?.note,
    ).toBe("Link function: Probit.");
    expect(
      formatted.sections.find((s) => s.id === "ordinal_parameter_estimates")
        ?.note,
  ...
  it("should include factor level counts in case processing summary", () => {
    ...
    const section = formatted.sections.find(
      (s) => s.id === "ordinal_case_processing_summary",
    );
    expect(section).toBeDefined();
    expect(section?.data.columnHeaders[1].header).toBe("Marginal Percentage");
    expect(section?.data.rows).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          rowHeader: ["Residence Type", "Tower Block"],
          n: 40,
          percent: "40.0%",
        }),
        expect.objectContaining({
          rowHeader: ["Residence Type", "Apartment"],
          n: 60,
          percent: "60.0%",
        }),
      ]),
