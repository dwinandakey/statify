...
/**
 * Komponen konten utama Jarque-Bera Test
 *
 * Mengelola state dan logika untuk:
 * - Pemilihan variabel test (tidak ada grouping variable)
 * - Opsi analisis (significance level, descriptives)
 * - Eksekusi analisis melalui Web Worker
 * - Tour guide interaktif
 */
const JarqueBeraTestContent: FC<Omit<BaseModalProps, 'containerType'> & { containerType?: "dialog" | "sidebar" }> = ({ onClose, containerType = "dialog" }) => {
    ...
    const handleOkClick = useCallback(() => {
        ...
        runAnalysis();
    }, [testVariables.length, runAnalysis]);
        ...
        <>
            ...
            <div className="px-6 py-3 border-t border-border flex items-center justify-between bg-secondary flex-shrink-0">
                ...
                <div className="flex gap-2">
                    ...
                    <Button
                        id="jarque-bera-ok-button"
                        onClick={handleOkClick}
                        disabled={isCalculating || testVariables.length === 0}
                    >
                        {isCalculating ? (
                            <>
...
const JarqueBeraTest: FC<BaseModalProps> = ({
    onClose,
...
export default JarqueBeraTest;
export { JarqueBeraTestContent };
